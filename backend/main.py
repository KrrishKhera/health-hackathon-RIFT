...
async def analyze(
    ...
    # FIX 1: Split comma-separated drug values and normalize
    # Handles both "CODEINE,WARFARIN" (single field) and repeated fields
    ...
    if "#CHROM" not in text:
        raise HTTPException(
            status_code=400,
            detail="Missing VCF column header line (#CHROM)."
        )
    ...
    # FIX 2: Iterate over each drug individually instead of treating
    # the whole list as a single dict key (which would always return "Unknown")
    all_results = []
    ...
    # FIX 3: Return a single result if only one drug was requested,
    # or a list if multiple drugs were analyzed — keeps backward compatibility
